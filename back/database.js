...
class Database {
    ...
    createSession(sessionData) {
        return new Promise((resolve, reject) => {
            const { user_id, mentor_id, topic, duration, rating, session_date } = sessionData;

            if (!user_id || !mentor_id || !topic || !duration) {
                return reject(new Error('Обязательные поля: user_id, mentor_id, topic, duration'));
            }

            const query = `
                INSERT INTO sessions (user_id, mentor_id, topic, duration, rating, session_date)
                VALUES (?, ?, ?, ?, ?, ?)
            `;

            this.db.run(query, [user_id, mentor_id, topic, duration, rating, session_date], function(err) {
                if (err) {
                    reject(err);
                } else {
                    resolve({
    ...
    updateSession(sessionId, updateData) {
        return new Promise((resolve, reject) => {
            const { topic, duration, rating } = updateData;
            const query = `
                UPDATE sessions 
                SET topic = ?, duration = ?, rating = ?
                WHERE id = ?
            `;

            this.db.run(query, [topic, duration, rating, sessionId], function(err) {
                if (err) {
                    reject(err);
                } else {
                    resolve({
